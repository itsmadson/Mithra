...
from mithra_api.models import Run, RunStatus, Label, Feature
...
def make_job(session) -> Run:
    job = Run(bbox_west=59.60, bbox_south=36.29, bbox_east=59.64, bbox_north=36.33)
    session.add(job)
    session.commit()
    return job
...
def test_new_job_starts_queued(session):
    assert make_job(session).status == RunStatus.QUEUED
...
def test_sign_stores_a_geographic_point(session):
    ...
    session.commit()
    assert session.scalar(select(Feature)).class_name == "street_name"
...
def test_label_attaches_to_a_sign(session):
    ...
    session.add(feature)
    session.commit()
    session.add(Label(feature_id=feature.id, class_name="city_entry"))
    session.commit()
    assert session.scalar(select(Label)).class_name == "city_entry"
